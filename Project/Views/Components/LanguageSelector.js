...
import { SPANISH_LANG, ENGLISH_LANG } from '../../Store/Reducers/DetailProfile';
...
class LanguageSelector extends React.Component {
  ...
  // usage of store
  toggleLanguage = () => {
    const toggledLang = store.getState().language === SPANISH_LANG ? ENGLISH_LANG : SPANISH_LANG;
    store.dispatch({
      type: LOAD_LANGUAGE,
      payload: toggledLang
    });
  ...
  render() {
    const langSrc = store.getState().language === SPANISH_LANG ? es_flag : en_flag;
